...
export default function Reports() {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      a.href = url; a.download = `attendance_${dateFrom}_to_${dateTo}.xlsx`; a.click();
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 20px' }} className="page-wrap">
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <h1 style={{ fontSize: '24px', fontWeight: '700', color: 'var(--ink)', margin: '0 0 6px', letterSpacing: '-0.02em' }}>Reports</h1>
      ...
      <div className="panel" style={{ marginBottom: '24px' }}>
        ...
        <form onSubmit={handleSearch} style={{ padding: '20px 22px' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'flex-end', marginBottom: '16px' }} className="filter-row">
            {[{ label: 'From', value: dateFrom, set: setDateFrom }, { label: 'To', value: dateTo, set: setDateTo }].map(({ label, value, set }) => (
              <div key={label}>
                ...
                <input type="date" value={value} onChange={e => set(e.target.value)} style={selectStyle}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            <button type="submit" className="btn-primary" style={{ width: 'auto', padding: '10px 22px' }}>Preview</button>
            {searched && preview.length > 0 && (
              <button type="button" onClick={handleDownload} disabled={downloading} className="btn-ghost" style={{ width: 'auto', padding: '10px 22px' }}>
      ...
      {/* Summary stats */}
      {searched && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '20px' }} className="stat-grid">
          {[
            { label: 'Total',    value: preview.length,         color: 'var(--ink)',      accent: '#a855f7' },
            { label: 'Present',  value: counts.Present,         color: 'var(--present)',  accent: '#4ade80' },
            { label: 'Late',     value: counts.Late,            color: 'var(--late)',     accent: '#fbbf24' },
            { label: 'Absent',   value: counts.Absent,          color: 'var(--absent)',   accent: '#f87171' },
            { label: 'On Leave', value: counts['On Leave'],     color: 'var(--on-leave)', accent: '#a78bfa' },
            ...
            <div key={label} className="stat-card" style={{ padding: '16px' }}>
              ...
              <p style={{ fontSize: '30px', fontWeight: '700', color, margin: 0, fontFamily: 'var(--mono)', lineHeight: 1, letterSpacing: '-0.02em' }}>{value}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {searched && (
        <div className="panel" style={{ marginBottom: '20px' }}>
          ...
          {loading ? (
            ...
            <p style={{ padding: '52px', textAlign: 'center', color: 'var(--ink-3)', fontFamily: 'var(--mono)', fontSize: '12px', margin: 0 }}>No records found for the selected range.</p>
          ) : (
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td style={{ fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--ink-2)' }}>
                          {formatTime(r.employees?.shift_start)}{r.employees?.shift_end && <span> – {formatTime(r.employees.shift_end)}</span>}
                        </td>
      ...
      {searched && preview.length > 0 && (
        <div className="panel">
          <div className="panel-header accent-bar"><span className="panel-title">Send Report by Email</span></div>
          <div style={{ padding: '20px 22px' }}>
            <p style={{ margin: '0 0 16px', fontSize: '13px', color: 'var(--ink-3)', lineHeight: 1.6 }}>Send the Excel report by email. Leave recipient blank to send to all employees.</p>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', flexWrap: 'wrap', marginBottom: '16px' }} className="email-row">
              ...
              <button type="button" onClick={handleSendEmail} disabled={sending} className="btn-primary" style={{ width: 'auto', padding: '12px 20px', whiteSpace: 'nowrap' }}>
                ✉ {sending ? 'Sending…' : emailRecipient ? 'Send to Employee' : 'Send to All'}
              </button>
            </div>
            {sendResult && <div className="msg-success"><p>Sent to {sendResult.sent_to} recipient{sendResult.sent_to !== 1 ? 's' : ''}.</p></div>}
            {sendError && <div className="msg-error"><p>{sendError}</p></div>}
          </div>
        </div>
      )}
    </div>
